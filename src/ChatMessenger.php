...
namespace Mahendraempyreal\EmpChat;
...
class ChatMessenger
{
    ...
    {
      return config('eichat.attachments.max_upload_size') * 1048576;
    }
    ...
    {
        return config('eichat.attachments.allowed_images');
    }
    ...
    {
        return config('eichat.attachments.allowed_files');
    }
    ...
    public function parseMessage($prefetchedMessage = null, $id = null)
    {
        ...
        return [
            ...
            'timeAgo' => \Carbon\Carbon::parse($msg->created_at)->setTimezone('Asia/Calcutta')->format("h:i A"),
    ...
    public function messageCard($data, $renderDefaultCard = false)
    {
        ...
        return view('empchat::chat.layouts.messageCard', $data)->render();
    }
    ...
    public function getContactItem($user)
    {
      try {
        ...
        return view('empchat::chat.layouts.listItem', [
            'get' => 'users',
    ...
    {
        if ($user->avatar == 'avatar.png' || is_null($user->avatar)) { // && config('eichat.gravatar.enabled')
            /* $imageSize = config('eichat.gravatar.image_size');
            $imageset = config('eichat.gravatar.imageset');
            $user->avatar = 'https://www.gravatar.com/avatar/' . md5(strtolower(trim($user->email))) . '?s=' . $imageSize . '&d=' . $imageset; */
            $user->avatar = self::getUserAvatarUrl('avatar.png');
        } else {
    ...
    public function getSharedPhotos($user_id)
    {
      ...
      // Get messages
      $msgs = $this->fetchMessagesQuery($user_id)->whereNotNull('attachment')->orderBy('created_at', 'DESC');
      if ($msgs->count() > 0) {
    ...
    public function deleteConversation($user_id)
    {
        try {
            foreach ($this->fetchMessagesQuery($user_id)->get() as $msg) {
                ...
                if (isset($msg->attachment)) {
                    $path = config('eichat.attachments.folder').'/'.json_decode($msg->attachment)->new_name;
    ...
    {
        return Storage::disk(config('eichat.storage_disk_name'));
    }
    ...
    {
        return self::storage()->url(config('eichat.user_avatar.folder') . '/' . $user_avatar_name);
    }
    ...
    {
        return self::storage()->url(config('eichat.attachments.folder') . '/' . $attachment_name);
    }
}
